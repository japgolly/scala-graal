...
An `Expr` is an expression/function/procedure, in some embedded language that can be evaluated by Graal.
...
// To evaluate Math.pow(2, 4) simply call your function
var pow_2_4: Expr[Int] =
  mathPow(2, 4)

// You don't need to prepare functions, you can evaluate expressions directly
...
// because we're not traversing an Expr, we're traversing Scala collection.
...
A `GraalContext` provides what GraalVM calls "Contexts" which are stateful sandboxes/environments
in which you execute your foreign language code.

To create synchronous ones:
...
* `GraalContext.fixedContext(options)` - creates a single context and uses it for all `.eval` calls.
  In order words, one `eval` call can modify state and affect subsequent calls.

* `GraalContext.newContextPerUse(options)` - creates a new, fresh context for each call to `.eval`
  In order words, `eval` calls are isolated and can't affect each other.
...
* `GraalContext.Builder.{fixedContext,newContextPerUse}(options).<config>.build()` -
  ...
  Configuration available:
    ...
    * `.useMutex(Boolean)` - whether or not eval calls need to be atomic, which avoids concurrency issues.
      Defaults to `true` for `fixedContext` and `false` for `newContextPerUse`.
...
For advanced users who want to output `F[Expr.Result[A]]` like `IO[Expr.Result[A]]` or anything else,
there's a `.trans` method that accepts a natural transformation.
