...
object ContextSync2 {
  ...
  def apply()(implicit l: Language): ContextSync2 = fixedContext()

  def fixedContext()(implicit l: Language): ContextSync2 = Builder.fixedContext().build()
  def fixedContext(c: Context): ContextSync2 = Builder.fixedContext(c).build()
  def newContextPerUse()(implicit l: Language): ContextSync2 = Builder.newContextPerUse().build()
  def newContextPerUse(c: => Context): ContextSync2 = Builder.newContextPerUse(c).build()
  def newContextPerUse(f: Engine => Context): ContextSync2 = Builder.newContextPerUse(f).build()
  ...
  private final class Impl(useMutex: Boolean,
                           ...
                           metricWriter: ContextMetrics.Writer) extends ContextSync2 {
